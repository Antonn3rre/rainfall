/* This file was generated by the Hex-Rays decompiler version 9.2.0.250908.
   Copyright (c) 2007-2021 Hex-Rays <info@hex-rays.com>

   Detected compiler: GNU C++
*/

#include <defs.h>


//-------------------------------------------------------------------------
// Function declarations

void (*init_proc())(void);
int sub_80483A0();
// int printf(const char *format, ...);
// char *fgets(char *s, int n, FILE *stream);
// time_t time(time_t *timer);
// char *strcpy(char *dest, const char *src);
// void *malloc(size_t size);
// int puts(const char *s);
// int __gmon_start__(void); weak
// int __libc_start_main(int (*main)(int, char **, char **), int argc, char **ubp_av, void (*init)(), void (*fini)(), void (*rtld_fini)(), void *stack_end);
// FILE *fopen(const char *filename, const char *modes);
// void __usercall __noreturn start(int a1@<eax>, void (*a2)()@<edx>);
void _do_global_dtors_aux();
int frame_dummy();
int m();
int __cdecl main(int argc, const char **argv, const char **envp);
void _libc_csu_fini(); // idb
void (*_do_global_ctors_aux())(void);
void term_proc();

//-------------------------------------------------------------------------
// Data declarations

_UNKNOWN _libc_csu_init;
int _CTOR_LIST__[] = { -1 }; // weak
int _DTOR_LIST__[] = { -1 }; // weak
int _DTOR_END__ = 0; // weak
int _JCR_LIST__ = 0; // weak
Elf32_Dyn *GLOBAL_OFFSET_TABLE_ = &DYNAMIC; // weak
int (*dword_8049910)(void) = NULL; // weak
char completed_6159; // weak
int dtor_idx_6161; // weak
char c[80]; // idb
// extern _UNKNOWN _gmon_start__; weak


//----- (0804836C) --------------------------------------------------------
// Alternative name is '_init'
void (*init_proc())(void)
{
  if ( &_gmon_start__ )
    __gmon_start__();
  frame_dummy();
  return _do_global_ctors_aux();
}
// 8048410: using guessed type int __gmon_start__(void);

//----- (080483A0) --------------------------------------------------------
int sub_80483A0()
{
  return dword_8049910();
}
// 8049910: using guessed type int (*dword_8049910)(void);

//----- (08048440) --------------------------------------------------------
// positive sp value has been detected, the output may be wrong!
void __usercall __noreturn start(int a1@<eax>, void (*a2)()@<edx>)
{
  int v2; // esi
  int v3; // [esp-4h] [ebp-4h] BYREF
  char *retaddr; // [esp+0h] [ebp+0h] BYREF

  v2 = v3;
  v3 = a1;
  __libc_start_main(
    (int (*)(int, char **, char **))main,
    v2,
    &retaddr,
    (void (*)())_libc_csu_init,
    _libc_csu_fini,
    a2,
    &v3);
  __halt();
}
// 8048443: positive sp value 4 has been found

//----- (08048470) --------------------------------------------------------
void _do_global_dtors_aux()
{
  int v0; // eax
  unsigned int i; // ebx

  if ( !completed_6159 )
  {
    v0 = dtor_idx_6161;
    for ( i = &_DTOR_END__ - _DTOR_LIST__ - 1; dtor_idx_6161 < i; v0 = dtor_idx_6161 )
    {
      dtor_idx_6161 = v0 + 1;
      ((void (*)(void))_DTOR_LIST__[v0 + 1])();
    }
    completed_6159 = 1;
  }
}
// 8049830: using guessed type int _DTOR_LIST__[];
// 8049834: using guessed type int _DTOR_END__;
// 8049940: using guessed type char completed_6159;
// 8049944: using guessed type int dtor_idx_6161;

//----- (080484D0) --------------------------------------------------------
int frame_dummy()
{
  int result; // eax

  result = _JCR_LIST__;
  if ( _JCR_LIST__ )
    return 0;
  return result;
}
// 8049838: using guessed type int _JCR_LIST__;

//----- (080484F4) --------------------------------------------------------
int m()
{
  time_t v0; // eax

  v0 = time(0);
  return printf("%s - %d\n", c, v0);
}

//----- (08048521) --------------------------------------------------------
int __cdecl main(int argc, const char **argv, const char **envp)
{
  FILE *v3; // eax
  void *v5; // [esp+18h] [ebp-8h]
  void *v6; // [esp+1Ch] [ebp-4h]

  v6 = malloc(8u);
  *(_DWORD *)v6 = 1;
  *((_DWORD *)v6 + 1) = malloc(8u);
  v5 = malloc(8u);
  *(_DWORD *)v5 = 2;
  *((_DWORD *)v5 + 1) = malloc(8u);
  strcpy(*((char **)v6 + 1), argv[1]);
  strcpy(*((char **)v5 + 1), argv[2]);
  v3 = fopen("/home/user/level8/.pass", "r");
  fgets(c, 68, v3);
  puts("~~");
  return 0;
}

//----- (08048610) --------------------------------------------------------
void __cdecl _libc_csu_init(int a1, int a2, int a3)
{
  int v3; // edi
  int i; // esi

  init_proc();
  v3 = (134519048 - (int)&GLOBAL_OFFSET_TABLE_) >> 2;
  if ( v3 )
  {
    for ( i = 0; i != v3; ++i )
      ((void (__cdecl *)(int, int, int))_CTOR_LIST__[i])(a1, a2, a3);
  }
}
// 8049828: using guessed type int _CTOR_LIST__[];
// 8049908: using guessed type Elf32_Dyn *GLOBAL_OFFSET_TABLE_;

//----- (08048690) --------------------------------------------------------
void (*_do_global_ctors_aux())(void)
{
  void (*result)(void); // eax
  void (**v1)(void); // ebx

  result = (void (*)(void))_CTOR_LIST__[0];
  if ( _CTOR_LIST__[0] != -1 )
  {
    v1 = (void (**)(void))_CTOR_LIST__;
    do
    {
      --v1;
      result();
      result = *v1;
    }
    while ( *v1 != (void (*)(void))-1 );
  }
  return result;
}
// 8049828: using guessed type int _CTOR_LIST__[];

//----- (080486BC) --------------------------------------------------------
// Alternative name is '_fini'
void term_proc()
{
  _do_global_dtors_aux();
}

// nfuncs=30 queued=10 decompiled=10 lumina nreq=0 worse=0 better=0
// ALL OK, 10 function(s) have been successfully decompiled
